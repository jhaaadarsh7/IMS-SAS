...
export function RequestsPageClient({
  ...
  const load = useCallback(() => {
    ...
    api
      .get<PaginatedResponse<BranchProductRequest>>("branch-requests", params)
      .then(setData)
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Request product from a warehouse for your branch. Admins approve and fulfill.
          </p>
        </div>
            ...
            <table className="table-dark">
              <thead>
                <tr>
                  ...
                  <th>Status</th>
                  <th>Notes</th>
                  {canUpdateStatus ? <th className="text-right">Actions</th> : null}
                </tr>
              </thead>
              <tbody>
                {data.items.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="text-slate-200 text-sm">{r.warehouse?.name || "—"}</td>
                    ...
                    <td>
                      <span className={`badge ${statusBadge[r.status] || "badge-slate"}`}>{r.status}</span>
                    </td>
                      ...
                      <td className="text-right">
                        {r.status === "PENDING" && (
                          <div className="flex justify-end gap-1">
                            <button
                              ...
                              className="btn btn-ghost btn-sm text-xs text-emerald-400"
                            >
                              Approve
                            </button>
                            <button
                        ...
                        {r.status === "APPROVED" && (
                          <button
                            ...
                            className="btn btn-ghost btn-sm text-xs text-sky-400"
                          >
                            Fulfill
                          </button>
                        )}
        ...
        {data && data.pagination.totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-white/[0.06]">
            ...
            <div className="flex gap-1">
              <button type="button" disabled={page <= 1} onClick={() => setPage(page - 1)} className="page-btn">
                ←
              </button>
              <button
                type="button"
...
function CreateRequestModal({
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setPending(true);
    try {
      await api.post("branch-requests", {
        branchId,
  ...
  return (
    <Modal open={open} onClose={onClose} title="Request product from warehouse">
      <form onSubmit={onSubmit} className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1">Your branch</label>
        ...
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1">Source warehouse</label>
          <select value={warehouseId} onChange={(e) => setWarehouseId(e.target.value)} className="input-dark">
          ...
          <p className="text-xs text-slate-500 mt-1">Admin sees which warehouse should supply this request.</p>
        </div>
        <div>
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          <button type="submit" disabled={pending} className="btn btn-primary">
            {pending ? <span className="spinner" style={{ width: 16, height: 16 }} /> : "Submit Request"}
          </button>
        </div>
      </form>
    </Modal>
